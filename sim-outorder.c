...
ruu_commit(void)
{
  ...
  while (RUU_num > 0 && committed < ruu_commit_width)
  {
    ...
    /* I think this is where we should update the value predictor's table. 
       Should be a similar process to bpred_update below -Pete*/
...
/* We can use this method to recover from value mispredictions as well, just like branch predictions. 
   It essentially just flushes the RUU back to the mis-speculated instruction -Pete*/
...
lsq_refresh(void)
{
 ...
 as no later load could be resolved in its presence */
    ...
    if (/* load? */
        ((MD_OP_FLAGS(LSQ[index].op) & (F_MEM | F_LOAD)) == (F_MEM | F_LOAD)) && /* queued? */ !LSQ[index].queued && /* waiting? */ !LSQ[index].issued && /* completed? */ !LSQ[index].completed && /* regs ready? */ OPERANDS_READY(&LSQ[index]))
    {
...
#include "machine.def"
    ...
    if ((pred_PC != regs.regs_NPC && pred_perfect) || ((MD_OP_FLAGS(op) & (F_CTRL | F_DIRJMP)) == (F_CTRL | F_DIRJMP) && target_PC != pred_PC && br_pred_taken))
    {
    ...
    if (!spec_mode)
    {
      ...
      /* This is where it checks whether a branch is mispredicted or not.
         I think we should do the same with our value prediction -Pete*/
...
ruu_fetch(void)
{
       ...
       i++)
  {
    ...
    /* This is where branch prediction is done I think we should do the same 
       with value prediction -Pete*/
    ...
    /* Down here, check if it is a load, then do our value prediction logic, 
    probably add a field abt predicted value to fetch_data -Pete*/
    ...
    ptrace_newstage(fetch_data[fetch_tail].ptrace_seq,
                    PST_IFETCH,
                    ((last_inst_missed ? PEV_CACHEMISS : 0) | (last_inst_tmissed ? PEV_TLBMISS : 0)));
...
#include "machine.def"
  ...
  /** This is where the simulation actually starts -Pete */
